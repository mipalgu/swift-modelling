...
struct CodeGenerationTests {
    ...
    func testGenerateFamiliesIntegration() async throws {
        ...
        let expectedFiles = [
            "Family.swift", "Member.swift", "FamiliesFactory.swift", "FamiliesPackage.swift",
        ]
        for fileName in expectedFiles {
            let fileURL = pkgDir.appendingPathComponent(fileName)
            #expect(
                FileManager.default.fileExists(atPath: fileURL.path),
                "Missing expected file: \(fileName)")
        }
    ...
    func testGenerateOrganisationIntegration() async throws {
        // Given: The organisation.ecore metamodel from swift-ecore dependency
        // Located in checkouts directory after dependency resolution
        let ecoreFile = try findDependencyResource(
            fileName: "organisation.ecore",
            dependencyName: "swift-ecore",
            relativePath: "Tests/ECoreTests/Resources/xmi"
        )
    ...
    func testGenerateMinimalWithEnums() async throws {
        // Given: The minimal.ecore metamodel from swift-ecore dependency
        // Located in checkouts directory after dependency resolution
        let ecoreFile = try findDependencyResource(
            fileName: "minimal.ecore",
            dependencyName: "swift-ecore",
            relativePath: "Tests/ECoreTests/Resources/xmi"
        )
